...
//set cookie 6개월 방문자 수 계산

// 최근 6개월 방문자 데이터 조회
//CURDATE에서부터 과거 5개월치를 포함!, DATE_FORMAT: 월별로 1일부터의 데이터 계산
$recent_sql = "
    SELECT DATE_FORMAT(visit_date, '%Y-%m') AS month, SUM(visitors) AS total_visitors
    FROM visitor_data
    WHERE visit_date >= DATE_FORMAT(DATE_ADD(LAST_DAY(CURDATE()), INTERVAL -5 MONTH), '%Y-%m-01')
    GROUP BY month
    ORDER BY month ASC
";

$monthlyData = [];
if ($result = $mysqli->query($recent_sql)) {
  while ($row = $result->fetch_assoc()) {
    $monthlyData[$row['month']] = (int)$row['total_visitors'];
  }
  $result->free();
}
// 데이터가 없을 경우 기본값 설정
if (empty($monthlyData)) {
  $monthlyData = array_fill_keys(
    array_map(
      fn($i) => date('Y-m', strtotime("-$i months")),
      range(5, 0) // 6개월
    ),
    0
  );
...
// 최신 데이터 배열 추출
$latestMonths = array_keys($monthlyData);
$latestCounts = array_values($monthlyData);

//최근 6개월 달만 배열로 저장
$latestMonthNames = array_map(
  fn($month) => date('n월', strtotime($month . "-01")),
  $latestMonths
);

// echo "오늘 방문자 수: " . $todayVisitorCount . "<br>";
// echo "최근 6개월:<br>";
// print_r($latestMonthNames);
// echo "<br>최근 6개월 방문자 데이터:<br>";
...
<div class="container">
  ...
  <div class="bottom_wrapper d-flex justify-content-between">
    ...
    <div class="bott_right d-flex flex-column justify-content-between">
      ...
      <div class="d-flex justify-content-between">
        <div>
          <div class="d-flex justify-content-between" id="chartData"
