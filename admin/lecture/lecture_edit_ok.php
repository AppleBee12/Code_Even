...
foreach ($detail_name as $id => $name) {
  ...
  $query_update_detail = "
      ...
      SET
          title = '$name',
          quiz_id = " . ($quiz_id !== null ? $quiz_id : "NULL") . ",
          test_id = " . ($test_id !== null ? $test_id : "NULL") . ",
          video_url = '$video_url'
      WHERE id = $id
  ";

  if (!$mysqli->query($query_update_detail)) {
    error_log("기존 강의 업데이트 실패: " . $mysqli->error . " | 쿼리: " . $query_update_detail);
  }
}
...
foreach ($new_names as $i => $name) {
  ...
  $query_new_detail = "
      INSERT INTO lecture_detail (
          ...
          $leid, '$name', " . ($quiz_id !== null ? $quiz_id : "NULL") . ", 
          " . ($test_id !== null ? $test_id : "NULL") . ", '$video_url', $video_order
      )
  ";

  if (!$mysqli->query($query_new_detail)) {
    error_log("새 강의 추가 실패: " . $mysqli->error . " | 쿼리: " . $query_new_detail);
  } else {
      ...
      if (move_uploaded_file($file_tmp_name, $file_path)) {
        ...
        if ($mysqli->query($query_file)) {
          ...
          if (!$mysqli->query($query_update_file_id)) {
            error_log("파일 ID 업데이트 실패: " . $mysqli->error);
          }
        } else {
          error_log("파일 저장 실패: " . $mysqli->error);
        }
      } else {
        error_log("파일 업로드 실패: $file_name");
      }
    }
  }
